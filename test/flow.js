...
describe('Should deploy contract and upload URI at the right places', () => {
    ...
    it('Should allow minter to mint multiple tokens', async () => {
        await contract.connect(owner).mintBatch(owner.address, URI, 5);

        const Last_URI_from_contract = await contract.tokenURI(5);

        expect(Last_URI_from_contract).to.equal(URI);
    })
