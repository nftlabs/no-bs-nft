...
const contractPath = (rest) => {
    const result = path.resolve(process.cwd(), 'contracts', ...rest);
    return result;
};
...
const findImports = (dependency) => {
    ...
    if (dependency === 'BidExecutor.sol') {
        return { contents: fs.readFileSync(contractPath([dependency]), 'utf8') };
        ...
        return { contents: fs.readFileSync(contractPath(['utils', dep[dep.length - 1]]), 'utf8') };
    } else if (dependency.includes('access')) {
        return { contents: fs.readFileSync(contractPath(['access', dep[dep.length - 1]]), 'utf8') };
    } else if (dependency.includes('security')) {
        return {
            contents: fs.readFileSync(contractPath(['security', dep[dep.length - 1]]), 'utf8'),
        };
    } else if (dependency.includes('ERC721')) {
        return { contents: fs.readFileSync(contractPath(['ERC721', dep[dep.length - 1]]), 'utf8') };
    } else {
...
export const compileERC721 = async () => {
    ...
    const input = {
        ...
        sources: {
            'NFT.sol': {
                content: fs.readFileSync(contractPath(['NFT.sol']), 'utf8'),
            },
            'BidExecutor.sol': {
                content: fs.readFileSync(contractPath(['BidExecutor.sol']), 'utf8'),
            },
        },
        settings: {
            outputSelection: {
