...
export default function useGasPrice(chainId = 1): any {
    ...
    const getCostEstimates = (price: number) => {
        const uploadCost: number = (uploadTransactionGas.current * price) / 10 ** 9;
        ...
        const formattedUploadCost = `${uploadCostDecimals[0]}.${
            uploadCostDecimals[1].length > 3
                ? uploadCostDecimals[1].slice(0, 6)
                : uploadCostDecimals[1]
        }`;

        const deployCostEthDecimals = deployCost.toString().split('.');

        const formattedDeployCost = `${deployCostEthDecimals[0]}.${
            deployCostEthDecimals[1].length > 3
                ? deployCostEthDecimals[1].slice(0, 3)
                : deployCostEthDecimals[1]
        }`;
